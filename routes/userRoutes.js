...
router.post("/user/login", async (req, res) => {
  ...
  if (!foundUser) {
    ...
    const newHash = SHA256(req.fields.password + foundUser.salt).toString(encBase64);
...
router.post("/user/update", isAuthenticated, async (req, res) => {
  try {
    ...
    if (!foundUser) {
      ...
      // Update image
      if (req.files.picture) {
        await cloudinary.api.delete_resources_by_prefix(`${foundUser.account.avatar.public_id}`);
      ...
      if (req.fields.password) {
        foundUser.hash = SHA256(req.fields.password + foundUser.salt).toString(encBase64);
      }
